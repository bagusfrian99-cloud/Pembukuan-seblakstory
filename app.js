...
function renderTransactions(){
  ...
  $('txCards').innerHTML=days.map(day=>{
    ...
    const items=rows.map(x=>{const xid=esc(String(x.id));return `<div class="txWrap"><div class="txCard ${x.type==='in'?'incomeCard':'expenseCard'}"><div class="txIcon">${x.type==='in'?'↑':'↓'}</div><div class="txMain"><b>${esc(x.name)}</b><small>${esc(String(x.date||'').replace('T',' '))}${x.note?` • ${esc(x.note)}`:''}</small></div><div class="txAmount ${x.type==='in'?'green':'red'}">${x.type==='in'?'+':'-'}${money(x.amount)}<span>${txPaymentMethod(x)}</span></div>${x.type==='out'&&x.photo?`<button type="button" class="receiptBtn" data-tx-id="${xid}" aria-label="Lihat struk" onclick="event.preventDefault();event.stopPropagation();openReceiptPhotoById(this.dataset.txId)">▣</button>`:''}<button type="button" class="moreBtn txActionBtn" data-tx-id="${xid}" aria-label="Aksi transaksi">⋮</button></div><div class="inlineTxActions" hidden><button type="button" class="txEditBtn" data-tx-id="${xid}">✎ Edit</button>${x.type==='out'&&x.photo?`<button type="button" class="txPhotoBtn" data-tx-id="${xid}" onclick="event.preventDefault();event.stopPropagation();openReceiptPhotoById(this.dataset.txId)">▣ Lihat Foto</button>`:''}<button type="button" class="danger txDeleteBtn" data-tx-id="${xid}">🗑 Hapus</button></div></div>`}).join('');
...
function renderBulkBuySelect(){const el=$("bulkBuySelectList");if(!el)return;el.innerHTML=bulkBuySelection.map((x,i)=>`<div class="bulkBuyCheck"><input type="checkbox" data-bulk-index="${i}" ${x.checked?"checked":""} onchange="toggleBulkBuy(${i},this.checked)"><span class="bulkBuyName"><b>${esc(x.name)}</b><small>rencana beli</small></span><div class="bulkQty"><button type="button" aria-label="Kurangi jumlah" onclick="changeBulkBuyQty(${i},-1)">−</button><input type="number" min="0" value="${Math.max(0,Number(x.buy)||0)}" aria-label="Jumlah pack ${esc(x.name)}" onchange="setBulkBuyQty(${i},this.value)" oninput="setBulkBuyQty(${i},this.value)"><button type="button" aria-label="Tambah jumlah" onclick="changeBulkBuyQty(${i},1)">+</button><span class="bulkQtyUnit">pack</span></div></div>`).join("");$("bulkBuySelectCount").textContent=`${bulkBuySelection.filter(x=>x.checked).length} barang dipilih`}
...
function setBulkBuyQty(i,value){const x=bulkBuySelection[i];if(!x)return;const n=Math.max(0,Math.floor(Number(value)||0));x.buy=n;if(n===0)x.checked=false;else if(!x.checked)x.checked=true;savePendingBulkSelection();const input=document.querySelector(`#bulkBuySelectList .bulkBuyCheck:nth-child(${i+1}) .bulkQty input`);if(input&&Number(input.value)!==n)input.value=n;$("bulkBuySelectCount").textContent=`${bulkBuySelection.filter(x=>x.checked).length} barang dipilih`}
function changeBulkBuyQty(i,delta){const x=bulkBuySelection[i];if(!x)return;setBulkBuyQty(i,(Number(x.buy)||0)+delta);renderBulkBuySelect()}
function bulkBuySelectAll(v){bulkBuySelection.forEach(x=>{x.checked=v;if(v&&(!Number(x.buy)||Number(x.buy)<1))x.buy=1;});savePendingBulkSelection();renderBulkBuySelect()}
